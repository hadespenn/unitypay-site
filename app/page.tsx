...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section network" id="network"><div className="section-heading"><div><Overline>{t.mapOverline}</Overline><h2>{t.mapTitle}</h2></div><p>{t.mapText}</p></div><WorldMap regions={t.regions}/><div className="region-grid">{t.regions.map(([name,text],i)=><article key={name}><span>0{i+1}</span><div><h3>{name}</h3><p>{text}</p></div></article>)}</div><p className="map-note">{t.mapNote}</p></section>
    <section className="section trust" id="capabilities"><div className="section-heading"><div><Overline>{t.trustOverline}</Overline><h2>{t.trustTitle}</h2></div><p>{t.trustText}</p></div><div className="trust-grid">{t.trusts.map(([title,text],i)=><article key={title}><span>0{i+1}</span><div className="trust-icon">{["◇","⇄","⌁"][i]}</div><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="section flow" id="flow"><div className="flow-heading"><Overline>{t.flowOverline}</Overline><h2>{t.flowTitle}<br/><em>{t.flowAccent}</em></h2></div><div className="flow-track">{t.steps.map(([title,text],i)=><article key={title}><div className="step-line"><b>0{i+1}</b><i/></div><small>{["ACQUIRING","VA BUFFER","ROUTING","SETTLEMENT"][i]}</small><h3>{title}</h3><p>{text}</p>{i<3&&<span className="next">→</span>}</article>)}</div></section>
    <section className="section compliance" id="compliance"><div className="compliance-heading"><Overline>{t.complianceOverline}</Overline><h2>{t.complianceTitle}</h2><p>{t.complianceText}</p></div><div className="boundary-grid">{t.boundaries.map(([title,text],i)=><article key={title}><span>0{i+1}</span><div className="check">✓</div><h3>{title}</h3><p>{text}</p></article>)}</div><p className="disclaimer">{t.disclaimer}</p></section>
